...
namespace Infrastructure.Services.Friendship
{
    public class FriendshipService : IFriendshipService
    {
        ...
        public async Task SendRequestAsync(string currentUserId, string addresseeId)
        {
            ...
            {
                var friendship = new Friendship1()
                {
        ...
        private async Task<Friendship1> GetFriendshipAsync(string requesterId, string addresseeId)
        => await this._data
            .Friendships
                .FirstOrDefaultAsync(i => i.RequesterId == requesterId &&
                                        i.AddresseeId == addresseeId);

        private async Task RemoveFriendshipAsync(Friendship1 friendship)
        {
            if (friendship != null)
            {
